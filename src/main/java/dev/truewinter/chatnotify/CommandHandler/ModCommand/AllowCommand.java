...
public class AllowCommand extends Command {
    ...
    public AllowCommand(JavaPlugin plugin, Database db, Notifier notifier) {
        this.plugin = plugin;
        this.db = db;
    ...
    public void handleCommand(CommandSender sender, String[] args) {
        if (args.length != 3) {
            sender.sendMessage(ChatColor.YELLOW + "Invalid usage. Correct usage: " + ChatColor.GOLD + "/cnm allow <player> <word>");
            return;
        }
        ...
        if (db.playerHasWord(playerUUID, args[2])) {
            ...
            db.setApproved(true, playerUUID, word.getWord());
            sender.sendMessage(ChatColor.GREEN + "Word approved");
            notifier.notifyPlayerAboutAllow(word.getUUID(), word.getWord());
            notifier.update();
        } else {
